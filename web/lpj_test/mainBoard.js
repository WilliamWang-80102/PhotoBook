//定义一些全局变量，以后会用得到
var photoParent = new Array();    //这个数组用于保存整个画布中所有图片的src,dx,dy,dw,dh,图片的中心坐标
...
function floatPhoto(float_x,float_y) {
    ...
    for (var i = 0; i < photoCount; i++) {
        if (Math.abs(float_x - photoParent[i][5]) <= (photoParent[i][3] / 2) && Math.abs(float_y - photoParent[i][6]) <= (photoParent[i][4] / 2)) {
    ...
    for (var i = photoCount - 1; i >= 0; i--) {
        ...
        if ((photoParent[i][0] == photoLAP[a - 1][0]) && (photoParent[i][1] == photoLAP[a - 1][1]) && (photoParent[i][2] == photoLAP[a - 1][2]) && (photoParent[i][3] == photoLAP[a - 1][3]) && (photoParent[i][4] == photoLAP[a - 1][4]) && (photoParent[i][5] == photoLAP[a - 1][5]) && (photoParent[i][6] == photoLAP[a - 1][6])) {
...
function dragPhoto(drag_x, drag_y) {
    for (var i = 0; i < photoCount; i++) {
        var tempX = photoParent[i][5] + (drag_x - photoParent[i][5]) * Math.cos(-photoParent[i][7]) - (drag_y - photoParent[i][6]) * Math.sin(-photoParent[i][7]);                                   //鼠标还原的临时坐标
        var tempY = photoParent[i][6] + (drag_x - photoParent[i][5]) * Math.sin(-photoParent[i][7]) + (drag_y - photoParent[i][6]) * Math.cos(-photoParent[i][7]);


        if (((Math.abs(tempX - photoParent[i][5]) <= photoParent[i][3] * 2 / 5) && (Math.abs(tempY - photoParent[i][6]) <= 1 / 2 * photoParent[i][4])) || ((Math.abs(tempY - photoParent[i][6]) <= photoParent[i][4] * 2 / 5) && (Math.abs(tempX - photoParent[i][5]) <= 1 / 2 * photoParent[i][3]))) {
...
//旋转函数
function rotatePhoto(rotate_x, rotate_y) {
